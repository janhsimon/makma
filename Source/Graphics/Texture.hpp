...
private:
	...
	static vk::Image *createImage(const std::shared_ptr<Context> context, uint32_t width, uint32_t height, vk::Format format);
	...
	static vk::ImageView *createImageView(const std::shared_ptr<Context> context, const vk::Image *image, vk::Format format);
