...
Mesh *Model::loadMeshData(const std::shared_ptr<Context> context, const std::shared_ptr<Buffers> buffers, const aiMesh *mesh, const aiMaterial *material, const std::string &path, const std::string &filename)
{
...
Model::Model(const std::shared_ptr<Context> context, const std::shared_ptr<Buffers> buffers, const std::string &path, const std::string &filename)
{
	...
	for (unsigned int i = 0; i < scene->mNumMeshes; ++i)
	{
		...
		meshes.push_back(loadMeshData(context, buffers, mesh, material, path, filename));
