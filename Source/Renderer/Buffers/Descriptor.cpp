...
vk::DescriptorSetLayout *Descriptor::createLayout(const std::shared_ptr<Context> context, vk::DescriptorType type, vk::ShaderStageFlags shaderStageFlags)
{
	auto layoutBinding = vk::DescriptorSetLayoutBinding().setDescriptorCount(1).setDescriptorType(type).setStageFlags(shaderStageFlags);
...
vk::DescriptorSet *Descriptor::createSet(const std::shared_ptr<Context> context, const std::shared_ptr<DescriptorPool> descriptorPool, const vk::DescriptorSetLayout *layout, const vk::Buffer *buffer, vk::DeviceSize range, vk::DescriptorType type)
{
	auto descriptorSetAllocateInfo = vk::DescriptorSetAllocateInfo().setDescriptorPool(*descriptorPool->getPool()).setDescriptorSetCount(1).setPSetLayouts(layout);
	...
	auto writeDescriptorSet = vk::WriteDescriptorSet().setDstSet(descriptorSet).setDescriptorType(type).setDescriptorCount(1).setPBufferInfo(&descriptorBufferInfo);
