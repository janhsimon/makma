...
public:
	...
	std::shared_ptr<Light> loadLight(LightType type, const glm::vec3 &position, const glm::vec3 &color = glm::vec3(1.0f, 1.0f, 1.0f), float range = 1000.0f, float intensity = 1.0f, float specularPower = 4.0f);
	...
	Buffers *getBuffers() const { return buffers.get(); }
};