...
void UniformBuffer::addDescriptor(const std::shared_ptr<DescriptorPool> descriptorPool, vk::ShaderStageFlagBits shaderStageFlags, vk::DeviceSize range)
{
	descriptors->push_back(std::make_unique<Descriptor>(context, descriptorPool, dynamic ? vk::DescriptorType::eUniformBufferDynamic : vk::DescriptorType::eUniformBuffer, shaderStageFlags, buffer->getBuffer(), range));
}