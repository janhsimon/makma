...
private:
  ...
  std::function<void(VkDebugReportCallbackEXT*)> debugReportCallbackDeleter =
    [this](VkDebugReportCallbackEXT* debugReportCallback) {
      // NOTE: instance.destroyDebugReportCallbackEXT() would be much nicer but is not currently implemented by the API
      // (only declared)
      PFN_vkDestroyDebugReportCallbackEXT vkDestroyDebugReportCallbackEXT =
        reinterpret_cast<PFN_vkDestroyDebugReportCallbackEXT>(instance->getProcAddr("vkDestroyDebugReportCallbackEXT"));
  ...
  std::function<void(vk::SurfaceKHR*)> surfaceDeleter = [this](vk::SurfaceKHR* surface) {
    if (instance)
  ...
  static vk::Device*
  createDevice(const vk::SurfaceKHR* surface, const vk::PhysicalDevice* physicalDevice, uint32_t& queueFamilyIndex);
  std::function<void(vk::Device*)> deviceDeleter = [](vk::Device* device) {
    if (device)
      device->destroy();
  };
  ...
  std::function<void(vk::CommandPool*)> commandPoolDeleter = [this](vk::CommandPool* commandPool) {
    if (device)
  ...
  std::function<void(vk::QueryPool*)> queryPoolDeleter = [this](vk::QueryPool* queryPool) {
    if (device)
...
public:
  ...
  vk::CommandPool* getCommandPoolOnce() const
  {
    return commandPoolOnce.get();
  }
  vk::CommandPool* getCommandPoolRepeat() const
  {
    return commandPoolRepeat.get();
  }
  ...
  uint32_t getUniformBufferDataAlignment() const
  {
    return uniformBufferDataAlignment;
  }
  uint32_t getUniformBufferDataAlignmentLarge() const
  {
    return uniformBufferDataAlignmentLarge;
  }
};