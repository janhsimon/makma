...
vk::ShaderModule* Shader::createShaderModule(std::shared_ptr<Context> context, const std::vector<char>& code)
{
  auto shaderModuleCreateInfo =
    vk::ShaderModuleCreateInfo().setCodeSize(code.size()).setPCode(reinterpret_cast<const uint32_t*>(&code[0]));
...
Shader::Shader(std::shared_ptr<Context> context, const std::string& filename, vk::ShaderStageFlagBits shaderStageFlags)
{
  ...
  shaderModule = std::unique_ptr<vk::ShaderModule, decltype(shaderModuleDeleter)>(createShaderModule(context, code),
                                                                                  shaderModuleDeleter);
}
