...
ShadowMap::ShadowMap(const std::shared_ptr<Context> context, const std::shared_ptr<DescriptorPool> descriptorPool, const std::shared_ptr<ShadowPipeline> shadowPipeline)
{
	...
	auto barrier = vk::ImageMemoryBarrier().setOldLayout(vk::ImageLayout::eUndefined).setNewLayout(vk::ImageLayout::eShaderReadOnlyOptimal).setImage(*depthImage);
	barrier.setSubresourceRange(vk::ImageSubresourceRange(vk::ImageAspectFlagBits::eDepth, 0, 1, 0, Settings::shadowMapCascadeCount));
...
void ShadowMap::recordCommandBuffer(const std::shared_ptr<VertexBuffer> vertexBuffer, const std::shared_ptr<IndexBuffer> indexBuffer, const vk::DescriptorSet *shadowMapCascadeViewProjectionMatricesDescriptorSet, const vk::DescriptorSet *geometryWorldMatrixDescriptorSet, const std::shared_ptr<ShadowPipeline> shadowPipeline, const std::vector<std::shared_ptr<Model>> *models, uint32_t shadowMapIndex, uint32_t numShadowMaps)
{
	...
	for (int i = 0; i < Settings::shadowMapCascadeCount; ++i)
	{
		...
		this->commandBuffer->bindDescriptorSets(vk::PipelineBindPoint::eGraphics, *pipelineLayout, 1, 1, shadowMapCascadeViewProjectionMatricesDescriptorSet, 1, &dynamicOffset);
		...
		for (uint32_t j = 0; j < models->size(); ++j)
		{
			...
			this->commandBuffer->bindDescriptorSets(vk::PipelineBindPoint::eGraphics, *pipelineLayout, 0, 1, geometryWorldMatrixDescriptorSet, 1, &dynamicOffset);
		...
		auto barrier = vk::ImageMemoryBarrier().setOldLayout(vk::ImageLayout::eDepthStencilReadOnlyOptimal).setNewLayout(vk::ImageLayout::eDepthStencilReadOnlyOptimal).setImage(*depthImage);
