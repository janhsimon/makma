...
std::shared_ptr<Texture> Material::defaultWhiteRGBATexture, Material::defaultBlackRTexture,
  Material::defaultNormalRGBTexture;
std::vector<std::shared_ptr<Material>> Material::materials;
uint32_t Material::numMaterials = 0;

vk::DescriptorSet* Material::createDescriptorSet(const std::shared_ptr<Context> context,
                                                 ...
                                                 const Material* material)
{
  auto descriptorSetAllocateInfo = vk::DescriptorSetAllocateInfo()
                                     ...
                                     .setPSetLayouts(descriptorPool->getMaterialLayout());
  auto descriptorSet = context->getDevice()->allocateDescriptorSets(descriptorSetAllocateInfo).at(0);

  auto diffuseDescriptorImageInfo = vk::DescriptorImageInfo()
                                      .setImageLayout(vk::ImageLayout::eShaderReadOnlyOptimal)
                                      .setImageView(*material->getDiffuseTexture()->getImageView())
                                      .setSampler(*material->getDiffuseTexture()->getSampler());
  auto diffuseSamplerWriteDescriptorSet = vk::WriteDescriptorSet()
                                            .setDstBinding(0)
  ...
  auto normalDescriptorImageInfo = vk::DescriptorImageInfo()
                                     .setImageLayout(vk::ImageLayout::eShaderReadOnlyOptimal)
                                     .setImageView(*material->getNormalTexture()->getImageView())
                                     .setSampler(*material->getNormalTexture()->getSampler());
  ...
  auto metallicDescriptorImageInfo = vk::DescriptorImageInfo()
                                       .setImageLayout(vk::ImageLayout::eShaderReadOnlyOptimal)
                                       .setImageView(*material->getMetallicTexture()->getImageView())
                                       .setSampler(*material->getMetallicTexture()->getSampler());
  auto metallicSamplerWriteDescriptorSet = vk::WriteDescriptorSet()
                                             .setDstBinding(2)
                                             .setDstSet(descriptorSet)
                                             .setDescriptorType(vk::DescriptorType::eCombinedImageSampler);
  metallicSamplerWriteDescriptorSet.setDescriptorCount(1).setPImageInfo(&metallicDescriptorImageInfo);

  auto roughnessDescriptorImageInfo = vk::DescriptorImageInfo()
                                        .setImageLayout(vk::ImageLayout::eShaderReadOnlyOptimal)
                                        .setImageView(*material->getRoughnessTexture()->getImageView())
                                        .setSampler(*material->getRoughnessTexture()->getSampler());
  auto roughnessSamplerWriteDescriptorSet = vk::WriteDescriptorSet()
                                              .setDstBinding(3)
                                              .setDstSet(descriptorSet)
                                              .setDescriptorType(vk::DescriptorType::eCombinedImageSampler);
  roughnessSamplerWriteDescriptorSet.setDescriptorCount(1).setPImageInfo(&roughnessDescriptorImageInfo);

  std::vector<vk::WriteDescriptorSet> writeDescriptorSets = { diffuseSamplerWriteDescriptorSet,
                                                              normalSamplerWriteDescriptorSet,
                                                              metallicSamplerWriteDescriptorSet,
                                                              roughnessSamplerWriteDescriptorSet };
