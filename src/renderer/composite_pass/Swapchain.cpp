...
vk::SwapchainKHR* Swapchain::createSwapchain(const std::shared_ptr<Window> window,
                                             ...
                                             vk::Extent2D& swapchainExtent)
{
  ...
  if ((surfaceCapabilities.minImageCount > imageCount || surfaceCapabilities.maxImageCount < imageCount) &&
...
Swapchain::createImageViews(const std::shared_ptr<Context> context, const std::vector<vk::Image>* images)
{
  auto imageViews = std::vector<vk::ImageView>(images->size());
  for (size_t i = 0; i < imageViews.size(); ++i)
  {
    auto imageViewCreateInfo = vk::ImageViewCreateInfo()
                                 .setImage(images->at(i))
...
vk::RenderPass* Swapchain::createRenderPass(const std::shared_ptr<Context> context)
{
  auto colorAttachmentDescription =
    vk::AttachmentDescription().setFormat(vk::Format::eB8G8R8A8Unorm).setLoadOp(vk::AttachmentLoadOp::eClear);
  ...
  auto renderPassCreateInfo =
    vk::RenderPassCreateInfo().setAttachmentCount(1).setPAttachments(&colorAttachmentDescription);
...
std::vector<vk::Framebuffer>* Swapchain::createFramebuffers(const std::shared_ptr<Context> context,
...
std::vector<vk::CommandBuffer>* Swapchain::createCommandBuffers(const std::shared_ptr<Context> context,
                                                                const std::vector<vk::Framebuffer>* framebuffers)
{
  auto commandBuffers = std::vector<vk::CommandBuffer>(framebuffers->size());
  auto commandBufferAllocateInfo = vk::CommandBufferAllocateInfo()
                                     .setCommandPool(*context->getCommandPoolRepeat())
                                     .setCommandBufferCount(static_cast<uint32_t>(commandBuffers.size()));

  if (context->getDevice()->allocateCommandBuffers(&commandBufferAllocateInfo, commandBuffers.data()) !=
      vk::Result::eSuccess)
  {
...
Swapchain::Swapchain(const std::shared_ptr<Window> window, const std::shared_ptr<Context> context)
{
  ...
  swapchain =
    std::unique_ptr<vk::SwapchainKHR, decltype(swapchainDeleter)>(createSwapchain(window, context, swapchainExtent),
                                                                  swapchainDeleter);
  ...
  framebuffers = std::unique_ptr<std::vector<vk::Framebuffer>, decltype(framebuffersDeleter)>(
    createFramebuffers(context, renderPass.get(), imageViews.get(), swapchainExtent), framebuffersDeleter);
  commandBuffers = std::unique_ptr<std::vector<vk::CommandBuffer>>(createCommandBuffers(context, framebuffers.get()));
}

void Swapchain::recordCommandBuffers(const std::shared_ptr<CompositePipeline> compositePipeline,
                                     ...
                                     const std::shared_ptr<UI> ui)
{
  ...
  std::array<vk::ClearValue, 4> clearValues = { vk::ClearColorValue(clearColor), vk::ClearColorValue(clearColor),
                                                vk::ClearColorValue(clearColor), vk::ClearDepthStencilValue(1.0f, 0) };
  ...
  for (size_t i = 0; i < commandBuffers->size(); ++i)
  {
    ...
    commandBuffer.bindDescriptorSets(vk::PipelineBindPoint::eGraphics, *compositePipeline->getPipelineLayout(), 0, 1,
                                     lightingBuffer->getDescriptorSet(), 0, nullptr);
