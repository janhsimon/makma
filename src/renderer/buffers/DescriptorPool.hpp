...
private:
  ...
  createPool(const std::shared_ptr<Context> context, uint32_t numTextures, uint32_t numShadowMaps);
  std::function<void(vk::DescriptorPool*)> poolDeleter = [this](vk::DescriptorPool* pool) {
  ...
  std::function<void(vk::DescriptorSetLayout*)> layoutDeleter = [this](vk::DescriptorSetLayout* layout) {
    if (context->getDevice())
      context->getDevice()->destroyDescriptorSetLayout(*layout);
  };
...
public:
  ...
  {
    return materialLayout.get();
  }
  vk::DescriptorSetLayout* getShadowMapLayout() const
  {
    return shadowMapLayout.get();
  }
  vk::DescriptorSetLayout* getGeometryBufferLayout() const
  {
    return geometryBufferLayout.get();
  }
  vk::DescriptorSetLayout* getLightingBufferLayout() const
  {
    return lightingBufferLayout.get();
  }
  vk::DescriptorSetLayout* getFontLayout() const
  {
    return fontLayout.get();
  }
};