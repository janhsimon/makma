...
SDL_Window* Window::createWindow(unsigned short width, unsigned short height, WindowMode mode)
{
  ...
  {
    throw std::runtime_error("Failed to initialize SDL video subsystem, encountered error: " +
  ...
  window =
    SDL_CreateWindow("Makma", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, width, height, SDL_WINDOW_VULKAN | flags);
